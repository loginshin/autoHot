; 원래 ||사이에 하나씩 기능 있었는데 코드 순서 때문에 보기 편하라고 정리안된모습
GList = 뒤로;(Browser_Back);{Browser_Back}|앞으로;(Browser_Forward);!{Browser_Forward}|창되살리기;(Ctrl+Shift+T);+^t||복사;(Ctrl+C);^c||숨긴 창 복구;(Win+Shift+M);#+M||종료;(Ctrl+W);^{w}|||붙여넣기;(Ctrl+V);^v|||||모든창 최소화;(Win+M);#m|사용 안 함;;
...
|실행취소;(Ctrl+Z);^z
|윈도우 탐색기;(Win+E);#e
|영구삭제;(Shift+Delete);+{Del}
|삭제;(Delete);{Del}
|최소화;(Alt+Space+N);!{Space}n
|최대화;(Alt+Space+X);!{Space}x
|새로고침;(F5);{F5}
|뒤로;(Alt+Left);!{Left}
*/
