...
SetTitleMatchMode, 2  ; 부분 일치 검색 사용
...
foundFunc(){
    ...
    ; 여러 Chrome 브라우저를 찾아 리스트에 저장합니다.
    WinGet, browsers, List, ahk_exe chrome.exe

    ; 각 Chrome 브라우저에서 스프링공작소 탭을 찾습니다.
    Loop, %browsers% {
        thisID := browsers%A_Index%

        ; 현재 포커스된 탭의 타이틀을 가져옵니다.
        WinGetTitle, currentTab, A
        ; Chrome 브라우저를 찾아 활성화합니다.
        WinActivate, ahk_id %thisID%
        If InStr(currentTab, "스프링 코딩 공작소") {
            found := true
            break
        }

        ; 탭 전환을 위한 단축키 Ctrl + Tab을 전송하기 전에 Ctrl 키를 누릅니다.
        Send, {Ctrl down}

        ; 크롬 브라우저의 탭을 순환하며 원하는 웹사이트를 찾습니다.
        Loop, 10 {
            ; Ctrl + Tab 키를 눌러 다음 탭으로 이동합니다.
            Send, {Tab}
            Sleep, 10
...
nextPage() {
    ...
    send :
    Sleep, 33
    ; sendinput, javascript: 사용시 오토핫키에서 한글이나온다. 인코딩 부분건들여야함
...
LoginGUI(){
    ...
    Gui, -Caption -Border +ToolWindow  ;GUI 옵션 사용 
    return


;버튼 클릭 이벤트 처리
ButtonThankyouLoGin확인:
Gui, Destroy
return
}
;버튼 이벤트 함수화
...
Github(){
    URL := "https://github.com/loginshin" ; 이동하려는 링크를 지정합니다.
    Run, %URL% ; 지정된 링크를 기본 웹 브라우저로 여는 데 사용됩니다.
}
